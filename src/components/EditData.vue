  ...
  <v-container class="editContact">
    ...
    <v-row align-content="center" class="profileImg">
      <v-container>
        ...
        <v-row>
          <v-col class="d-flex justify-center" cols="12">
            <v-file-input v-model="newProfileImage" density="compact"
              accept="image/png, image/jpeg, image/bmp" label="Avatar" placeholder="Subir imagen" variant="outlined"
              prepend-icon="mdi-camera" class="imageInput"></v-file-input>
          </v-col>
        </v-row>
      </v-container>
    </v-row>
    <v-row class="noMarginTop">
      <v-container>
        ...
        <v-text-field density="compact" :label="user.firstName" variant="outlined" clearable
          v-model="newFirstName"></v-text-field>
        <v-text-field density="compact" :label="user.lastName" variant="outlined" clearable
          v-model="newLastName"></v-text-field>
        <v-text-field density="compact" :label="user.alias" variant="outlined" clearable
          v-model="newAlias"></v-text-field>
        <h3 class="title">Cambiar Contraseña:</h3>
        <v-text-field density="compact" v-model="newPassword" :type="showPassword ? 'text' : 'password'"
          label="Nueva Contraseña" variant="outlined" clearable :append-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'"
          @click:append="togglePasswordVisibility('new')"></v-text-field>
        <v-text-field density="compact" v-model="repeatPassword" :type="showRepeatPassword ? 'text' : 'password'"
          label="Repetir Contraseña" variant="outlined" clearable
          :append-icon="showRepeatPassword ? 'mdi-eye-off' : 'mdi-eye'"
          @click:append="togglePasswordVisibility('repeat')"></v-text-field>
      </v-container>
...
function handleConfirm() {
  ...
  if(usersStore.getUserByAlias(newAlias.value)) {
    snackbarMessage.value = 'Ya existe un usuario con ese alias'
    snackbar.value = true
    return
  }
